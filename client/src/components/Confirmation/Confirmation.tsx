...
const Confirmation = () => {
    ...
    const totalPrice = localCartItems.reduce((total, localCartItem) => total + (localCartItem.price * localCartItem.quantity), 0);
              ...
              {localCartItems.map((localCartItems, index) => (
                  <div className="orderItemCard" key={index}>
                      {localCartItems.img && localCartItems.img.length > 0 ? (
                          <img src={localCartItems.img[0]} alt={localCartItems.title} className="productImage"/>
                      ) : (
